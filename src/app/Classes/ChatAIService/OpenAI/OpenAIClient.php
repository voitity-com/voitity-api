...
class OpenAIClient implements ChatAIClient
{
    ...
    private function buildSystemPrompt(
        ...
        if ($publicSocialLinks !== null) {
            ...
            $prompt .= '. If asked directly or indirectly for one of these networks, accounts, channels, repositories, professional profiles, short-video accounts, long-video channels, or where to follow the profile, answer naturally using the matching record. Never substitute a different provider. Do not include the raw URL in answer. Add a reference with type "social_link" and its exact provider_key so the app can render the localized button. Treat these links as available profile information and do not say you do not have them';
        }
        ...
        if (is_string($productsPrompt)) {
            ...
            if ($productRecommendationGuidance !== null) {
                ...
                $prompt .= '. This guidance is authoritative for selecting which published product fits a stated need. Follow its routing whenever the visitor explicitly asks for a recommendation, comparison, choice, what fits their situation, or states a concrete goal or need that a retrieved product directly solves. It never changes or overrides published product facts, safety rules, or factual restrictions. For a concrete goal or need with a clearly relevant retrieved product, recommend it naturally and immediately set product_action to "show" with its product id, even when the visitor did not use words such as product, buy, or recommend. Do not show products for a merely topical mention, weak semantic relationship, greeting, or unrelated conversation. When one question contains several needs mapped to different products, name and attach every matching product. Direct product mentions and explicit product requests keep the existing immediate recommendation behavior';
                $prompt .= '. Apply the guidance progressively across recent messages. If the guidance requires qualifying questions, collect the missing information first. As soon as the available conversation satisfies the guidance or the visitor directly asks to hire, book, receive, or use the represented service, set product_request to true and show the matching product without waiting for product-specific wording';
            }
        }
        ...
        $prompt .= '. Return a JSON object only, without surrounding text. The JSON object must have exactly these keys: "answer" as the natural-language answer string, "media_request" as a boolean, "media_action" as "none" or "show", "media_ids" as an array of selected media ids to attach, "product_request" as a boolean, "product_action" as "none" or "show", "product_ids" as an array of published product ids to attach, "references" as an array of objects with exactly "type" and "id", and "constraints" as an object with include_providers, exclude_providers, include_source_types, exclude_source_types arrays and require_unseen boolean. Reference type must be social_link, integration_media, or product. Use the social provider_key as the social_link id and the numeric record id for media or products. Include only records actually used in the answer and intended for a button or card. Use "media_action":"none" and an empty media_ids array when no media should be attached. Use "product_action":"none" and an empty product_ids array when no product should be attached.';
    ...
    private function buildPublicSocialLinksPrompt(
        ...
        foreach ($networks as $network => $url) {
            ...
            $links[] = [
                'provider_key' => (string) $network,
                'provider_label' => $this->socialNetworkName((string) $network),
                'url' => $url,
            ];
        }
        ...
        $json = json_encode($links, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
