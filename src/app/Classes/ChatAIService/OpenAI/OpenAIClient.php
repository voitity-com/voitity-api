    ...
    private function buildSystemPrompt(
        ...
        if (is_string($productsPrompt)) {
            ...
            if ($productRecommendationGuidance !== null) {
                ...
                $prompt .= '. This guidance is authoritative for selecting which published product fits a stated need. Follow its routing whenever the visitor explicitly asks for a recommendation, comparison, choice, or what fits their situation. It never changes or overrides published product facts, safety rules, or factual restrictions. If the visitor only mentions a broad interest or need covered by the guidance but has not asked to see, buy, compare, choose, or receive a product recommendation, offer help naturally and keep product_action as "none" with an empty product_ids array. If the visitor accepts that offer or explicitly requests a recommendation, show the product or products mapped by the guidance. When one question contains several needs mapped to different products, name and attach every matching product. Direct product mentions and explicit product requests keep the existing immediate recommendation behavior';
            }
        }
